...
export async function GET(request: NextRequest) {
  try {
    ...
    const type = searchParams.get('type') || 'all' // internal, external, notifications, tasks
    ...
    // Use internal_messages table instead of communications
    let query = supabase
...
export async function POST(request: NextRequest) {
  try {
    ...
    // Create communication using internal_messages table
    const { data: communication, error } = await supabase
      .from('internal_messages')
      .insert([{
        message_type: body.type || 'general', // general, urgent, announcement, task, support
        ...
        is_admin_message: body.is_admin_message || false
      }])
      .select()
      .single()
