...
import { Plus, Search, Filter, Download, MoreHorizontal, CheckCircle, Eye, Edit, FileText, MessageSquare, Calendar, User, Clock, AlertTriangle, XCircle, Grid3X3, List, ArrowRight, Printer, Users, ArrowLeft } from "lucide-react"
...
const mockDeliveryData: DeliveryItem[] = [
  ...
  },
  {
    ...
    priority: 'medium',
    assignee: 'David Chen',
    notes: 'Standard delivery for gold pendant with confirmation.',
    progress: 90,
  ...
  },
  {
    ...
    notes: 'Successfully delivered mixed metal bracelet.',
    progress: 100,
  ...
  },
  {
    ...
    notes: 'Wedding band delivery failed - customer not available.',
    progress: 60,
  ...
  },
  {
    ...
    priority: 'high',
    assignee: 'Lisa Wang',
    notes: 'Express delivery for diamond stud earrings with signature required.',
    progress: 45,
    timeRemaining: '1 day',
  },
  {
    ...
    notes: 'Standard delivery for freshwater pearl necklace with confirmation.',
    progress: 85,
...
export default function DeliveryKanban() {
  const [items, setItems] = React.useState<DeliveryItem[]>(mockDeliveryData)
  ...
  const handleStatusChange = (itemId: string, newStatus: string) => {
    setItems(prev => prev.map(item => 
      item.id === itemId ? { ...item, status: newStatus as DeliveryItem["status"] } : item
    ))
    setEditingItem(null)
  }
  ...
  const handlePriorityChange = (itemId: string, newPriority: string) => {
    setItems(prev => prev.map(item => 
      item.id === itemId ? { ...item, priority: newPriority as DeliveryItem["priority"] } : item
    ))
    setEditingItem(null)
  }
  ...
  const exportToCSV = () => {
    ...
    a.href = url
    a.download = "delivery-kanban.csv"
    a.click()
      ...
      <div className="relative z-10 max-w-7xl mx-auto px-4 md:px-0 pt-10 pb-4">
        <div className="flex items-center justify-between mb-8 bg-white/80 backdrop-blur-xl rounded-2xl shadow-xl border border-emerald-200/50 p-6">
          <div className="flex items-center gap-4">
            ...
            <div>
              <h1 className="text-3xl font-extrabold bg-gradient-to-r from-emerald-700 to-green-700 bg-clip-text text-transparent tracking-tight">Delivery</h1>
              <p className="text-md text-emerald-600 font-medium">Manage and track all deliveries</p>
            </div>
          </div>
          <div className="flex flex-col items-end gap-2 min-w-[160px]">
            ...
            <div className="flex gap-2 mt-1">
              <Link href="/dashboard/logistics/kanban/pickup">
                <Button variant="ghost" size="icon" className="h-8 w-8 text-emerald-700 hover:bg-emerald-100 hover:text-emerald-800 rounded-full" title="Back to Pickup">
        ...
        <div className="flex justify-center gap-4 flex-wrap px-4 md:px-0">
          ...
          <Card className={`w-52 flex flex-col items-center justify-center p-4 cursor-pointer border-2 shadow-xl rounded-2xl bg-green-100 transition-all duration-300 ${statusFilter === 'approved' ? 'border-green-600 ring-2 ring-green-200 scale-105' : 'border-transparent hover:border-green-200 hover:scale-105'}`}
            ...
            <CardContent className="flex flex-col items-center justify-center p-0">
              ...
              <div className="text-xs font-medium mt-1 text-green-700">Delivered</div>
            </CardContent>
          </Card>
        </div>
      </div>
              ...
              <table className="w-full bg-white/60 luxury-table">
                ...
                <tbody>
                  {filteredItems.length === 0 ? (
                    <tr>
                      <td colSpan={10} className="text-center py-8 text-emerald-400">No delivery orders found.</td>
                    </tr>
                  ) : (
                          ...
                          <div className="flex items-center gap-2 justify-center">
                            <DropdownMenu>
                              ...
                              <DropdownMenuContent align="end" className="w-48 rounded-xl shadow-xl bg-white/95 border-emerald-200">
                                ...
                                <DropdownMenuItem onClick={() => alert('Mark as Delivered')}>Mark as Delivered</DropdownMenuItem>
                                <DropdownMenuItem onClick={() => alert('Print Delivery Slip')}><Printer className="w-4 h-4 mr-2" /> Print Delivery Slip</DropdownMenuItem>
                              </DropdownMenuContent>
                            </DropdownMenu>
                            <DropdownMenu>
                              <DropdownMenuTrigger asChild>
