...
const mockPackingDetail: PackingDetail = {
  id: 'ORD-1001',
  ...
  notes: 'Premium packaging for platinum engagement ring with diamond accents. Customer requested special presentation box with velvet lining.',
  progress: 75,
  specifications: {
    material: 'Platinum',
    size: 'Ring Size 6.5',
    ...
    specialRequirements: 'Velvet lining, certificate holder, cleaning cloth included'
  },
  timeline: [
    { date: 'Jul 20', event: 'Order received for packing', user: 'System' },
    { date: 'Jul 21', event: 'Assigned to Sarah Johnson', user: 'Manager' },
    { date: 'Jul 22', event: 'Packaging materials prepared', user: 'Sarah Johnson' },
    { date: 'Jul 23', event: 'Quality check initiated', user: 'Quality Team' },
    { date: 'Jul 24', event: 'Packaging in progress', user: 'Sarah Johnson' }
  ],
  files: [
    { name: 'packaging_instructions.pdf', type: 'PDF', size: '2.3 MB', uploadedBy: 'Sarah Johnson', date: 'Jul 22' },
    { name: 'quality_checklist.pdf', type: 'PDF', size: '1.1 MB', uploadedBy: 'Quality Team', date: 'Jul 23' },
    { name: 'packaging_photos.jpg', type: 'Image', size: '4.7 MB', uploadedBy: 'Sarah Johnson', date: 'Jul 24' }
  ],
  qualityChecks: [
    { check: 'Packaging materials quality', status: 'passed', notes: 'All materials meet premium standards', checkedBy: 'Quality Team', date: 'Jul 23' },
    { check: 'Certificate placement', status: 'pending', notes: 'Awaiting final packaging completion', checkedBy: 'Sarah Johnson', date: 'Jul 24' },
    { check: 'Final presentation check', status: 'pending', notes: 'To be completed before shipping', checkedBy: 'Quality Team', date: 'Jul 25' }
  ],
  relatedOrders: [
...
export default function PackingDetailPage() {
  ...
  const [detail, setDetail] = React.useState<PackingDetail>(mockPackingDetail)
  const [isEditing, setIsEditing] = React.useState(false)
  const [editForm, setEditForm] = React.useState<PackingDetail>(mockPackingDetail)
    ...
    <div className="flex-1 space-y-4 p-4 md:p-8 pt-6">
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center space-x-4">
          ...
          <div>
            <h2 className="text-3xl font-bold tracking-tight">Packing Detail</h2>
                  ...
                  {isEditing ? (
                    <Input 
                      ...
                      onChange={(e) => setEditForm({
                        ...editForm, 
                        specifications: {...editForm.specifications, material: e.target.value}
                      })}
                    />
                  ) : (
                    <p className="text-sm text-muted-foreground">{detail.specifications.material}</p>
                  )}
                </div>
                <div>
                  ...
                  {isEditing ? (
                    <Input 
                      value={editForm.specifications.size} 
                      onChange={(e) => setEditForm({
                        ...editForm, 
                        specifications: {...editForm.specifications, size: e.target.value}
                      })}
                    />
                  ) : (
                    <p className="text-sm text-muted-foreground">{detail.specifications.size}</p>
                  )}
                </div>
                <div>
                  ...
                  {isEditing ? (
                    <Input 
                      ...
                      onChange={(e) => setEditForm({
                        ...editForm, 
                        specifications: {...editForm.specifications, weight: e.target.value}
                      })}
                    />
                  ) : (
                    <p className="text-sm text-muted-foreground">{detail.specifications.weight}</p>
                  )}
                </div>
                <div>
                  ...
                  {isEditing ? (
                    <Input 
                      value={editForm.specifications.packagingType} 
                      onChange={(e) => setEditForm({
                        ...editForm, 
                        specifications: {...editForm.specifications, packagingType: e.target.value}
                      })}
                    />
                  ) : (
                    <p className="text-sm text-muted-foreground">{detail.specifications.packagingType}</p>
                  )}
                </div>
              </div>
              <div>
                ...
                {isEditing ? (
                  <Textarea 
                    value={editForm.specifications.specialRequirements} 
                    onChange={(e) => setEditForm({
                      ...editForm, 
                      specifications: {...editForm.specifications, specialRequirements: e.target.value}
                    })}
                    rows={3}
                  />
                ) : (
                  <p className="text-sm text-muted-foreground">{detail.specifications.specialRequirements}</p>
                )}
              </div>
            </CardContent>
          </Card>
