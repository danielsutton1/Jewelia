...
const mockShippingDetail: ShippingDetail = {
  id: 'ORD-1001',
  ...
  notes: 'Express shipping for platinum engagement ring with insurance. Customer requested signature confirmation and tracking updates.',
  progress: 75,
  specifications: {
  ...
  timeline: [
    { date: 'Jul 25', event: 'Order received for shipping', user: 'System' },
    { date: 'Jul 25', event: 'Assigned to Sarah Johnson', user: 'Manager' },
    { date: 'Jul 25', event: 'Shipping label generated', user: 'Sarah Johnson' },
    { date: 'Jul 26', event: 'Package prepared for pickup', user: 'Sarah Johnson' },
    { date: 'Jul 26', event: 'Carrier pickup scheduled', user: 'Sarah Johnson' }
  ],
  files: [
    { name: 'shipping_label.pdf', type: 'PDF', size: '1.2 MB', uploadedBy: 'Sarah Johnson', date: 'Jul 25' },
    { name: 'insurance_certificate.pdf', type: 'PDF', size: '0.8 MB', uploadedBy: 'Sarah Johnson', date: 'Jul 25' },
    { name: 'tracking_info.pdf', type: 'PDF', size: '0.5 MB', uploadedBy: 'Sarah Johnson', date: 'Jul 26' }
  ],
  qualityChecks: [
    { check: 'Package integrity', status: 'passed', notes: 'Package properly sealed and labeled', checkedBy: 'Quality Team', date: 'Jul 26' },
    { check: 'Insurance verification', status: 'passed', notes: 'Insurance coverage confirmed', checkedBy: 'Sarah Johnson', date: 'Jul 25' },
    { check: 'Tracking number validation', status: 'pending', notes: 'Awaiting carrier pickup', checkedBy: 'Sarah Johnson', date: 'Jul 26' }
  ],
  relatedOrders: [
...
export default function ShippingDetailPage() {
  ...
  const [detail, setDetail] = React.useState<ShippingDetail>(mockShippingDetail)
  const [isEditing, setIsEditing] = React.useState(false)
  const [editForm, setEditForm] = React.useState<ShippingDetail>(mockShippingDetail)
    ...
    <div className="flex-1 space-y-4 p-4 md:p-8 pt-6">
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center space-x-4">
          ...
          <div>
            <h2 className="text-3xl font-bold tracking-tight">Shipping Detail</h2>
      ...
      <Tabs defaultValue="overview" className="space-y-4">
        <TabsList>
          ...
          <TabsTrigger value="specifications">Shipping Details</TabsTrigger>
        ...
        <TabsContent value="specifications" className="space-y-4">
          <Card>
            <CardHeader>
              <CardTitle>Shipping Specifications</CardTitle>
            </CardHeader>
            <CardContent className="space-y-4">
              <div className="grid gap-4 md:grid-cols-2">
                <div>
                  ...
                  {isEditing ? (
                    <Input 
                      value={editForm.specifications.shippingMethod} 
                      onChange={(e) => setEditForm({
                        ...editForm, 
                        specifications: {...editForm.specifications, shippingMethod: e.target.value}
                      })}
                    />
                  ) : (
                    <p className="text-sm text-muted-foreground">{detail.specifications.shippingMethod}</p>
                  )}
                </div>
                <div>
                  ...
                  {isEditing ? (
                    <Input 
                      value={editForm.specifications.trackingNumber} 
                      onChange={(e) => setEditForm({
                        ...editForm, 
                        specifications: {...editForm.specifications, trackingNumber: e.target.value}
                      })}
                    />
                  ) : (
                    <p className="text-sm text-muted-foreground">{detail.specifications.trackingNumber}</p>
                  )}
                </div>
                <div>
                  ...
                  {isEditing ? (
                    <Input 
                      value={editForm.specifications.insurance} 
                      onChange={(e) => setEditForm({
                        ...editForm, 
                        specifications: {...editForm.specifications, insurance: e.target.value}
                      })}
                    />
                  ) : (
                    <p className="text-sm text-muted-foreground">{detail.specifications.insurance}</p>
                  )}
                </div>
                <div>
                  ...
                  {isEditing ? (
                    <Input 
                      value={editForm.specifications.signatureRequired} 
                      onChange={(e) => setEditForm({
                        ...editForm, 
                        specifications: {...editForm.specifications, signatureRequired: e.target.value}
                      })}
                    />
                  ) : (
                    <p className="text-sm text-muted-foreground">{detail.specifications.signatureRequired}</p>
                  )}
                </div>
              </div>
              <div>
