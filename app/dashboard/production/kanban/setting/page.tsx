...
import { Gem, Calendar, Sparkles, Search, Download, Printer, ChevronDown, ChevronUp, Filter, Plus, CheckCircle, XCircle, Users, ArrowDownCircle, Paperclip, Clock, AlertTriangle, ArrowRight, ArrowLeft, MessageSquare, MoreHorizontal } from "lucide-react";
...
const mockSettingOrders = [
  {
    ...
    revisions: 1,
    files: [{ name: "setting-instructions.pdf" }, { name: "stone-specs.pdf" }],
  ...
  },
  {
    ...
    revisions: 0,
    files: [{ name: "final-setting.stl" }, { name: "setting-report.pdf" }],
  ...
  },
  {
    ...
    revisions: 3,
    files: [{ name: "revision-notes.pdf" }, { name: "updated-setting.stl" }],
...
const statusSummary = [
  { key: "all", label: "Total Settings", value: mockSettingOrders.length, color: "bg-emerald-100 text-emerald-800", icon: Gem },
  { key: "In Progress", label: "In Progress", value: mockSettingOrders.filter(s => s.status === "In Progress").length, color: "bg-blue-100 text-blue-800", icon: Clock },
  { key: "Review", label: "Under Review", value: mockSettingOrders.filter(s => s.status === "Review").length, color: "bg-yellow-100 text-yellow-800", icon: AlertTriangle },
  { key: "Approved", label: "Approved", value: mockSettingOrders.filter(s => s.status === "Approved").length, color: "bg-green-100 text-green-800", icon: CheckCircle },
  { key: "Revise", label: "Revise", value: mockSettingOrders.filter(s => s.status === "Revise").length, color: "bg-red-100 text-red-800", icon: XCircle },
];
...
const assignees = Array.from(new Set(mockSettingOrders.map(s => s.assignedTo)));
...
export default function SettingStagePage() {
  ...
  const [settingOrders, setSettingOrders] = useState(mockSettingOrders);
  const router = useRouter();

  const filteredSettingFiles = mockSettingOrders.filter(setting => {
    if (activeSummary !== "all" && setting.status !== activeSummary) return false;
    if (search && !setting.customerName.toLowerCase().includes(search.toLowerCase()) && !setting.settingId.toLowerCase().includes(search.toLowerCase())) return false;
    if (dateRange && dateRange.from && dateRange.to) {
      const settingDate = parseISO(setting.dueDate);
      if (!isWithinInterval(settingDate, { start: dateRange.from, end: dateRange.to })) return false;
    }
    if (assignee && setting.assignedTo !== assignee) return false;
    return true;
  });
  ...
  const sortedSettingFiles = [...filteredSettingFiles].sort((a, b) => {
    let aValue: any;
    let bValue: any;
  ...
  // Analytics
  const totalSettingFiles = sortedSettingFiles.length;
  const settingFilesByStaff = staffList.map(staff => ({ staff, count: sortedSettingFiles.filter(setting => setting.assignedTo === staff).length }));

  // Useful analytics
  const avgRevisions = sortedSettingFiles.length ? (sortedSettingFiles.reduce((sum, s) => sum + s.revisions, 0) / sortedSettingFiles.length).toFixed(1) : 0;
  ...
  function handleDownloadPDF(setting: any) {
    const doc = new jsPDF();
    let y = 20;
    doc.setFontSize(18);
    doc.text("Setting File Detail", 10, y);
    y += 10;
    doc.setFontSize(12);
    doc.text(`Setting ID: ${setting.settingId}`, 10, y); y += 8;
    doc.text(`Customer: ${setting.customerName} (${setting.customerId || ''})`, 10, y); y += 8;
    doc.text(`Status: ${setting.status}`, 10, y); y += 8;
    doc.text(`Priority: ${setting.priority}`, 10, y); y += 8;
    doc.text(`Assigned To: ${setting.assignedTo}`, 10, y); y += 8;
    doc.text(`Due Date: ${format(parseISO(setting.dueDate), "PPpp")}`, 10, y); y += 8;
    doc.text(`Revisions: ${setting.revisions}`, 10, y); y += 8;
    doc.text(`Order Number: ${setting.orderNumber}`, 10, y); y += 8;
    doc.text(`Casting ID: ${setting.castingId}`, 10, y); y += 8;
    doc.text("Notes:", 10, y); y += 6;
    doc.setFontSize(11);
    doc.text(doc.splitTextToSize(setting.notes || '', 180), 12, y); y += 12 + 6 * Math.ceil((setting.notes || '').length / 80);
    doc.save(`${setting.settingId}.pdf`);
  }
      ...
      <div className="relative z-10 max-w-7xl mx-auto px-4 md:px-0 pt-10 pb-4">
        <div className="flex items-center justify-between mb-8 bg-white/80 backdrop-blur-xl rounded-2xl shadow-xl border border-emerald-200/50 p-6">
          <div className="flex items-center gap-4">
            ...
            <div>
              <h1 className="text-3xl font-extrabold bg-gradient-to-r from-emerald-700 to-green-700 bg-clip-text text-transparent tracking-tight">Setting Stage Management</h1>
              <p className="text-md text-emerald-600 font-medium">Track and manage all setting stage orders</p>
            </div>
          </div>
          <div className="flex flex-col items-end gap-2 min-w-[160px]">
            <Button className="bg-gradient-to-r from-emerald-600 to-green-600 text-white px-6 py-2 rounded-xl font-semibold shadow-lg hover:from-emerald-700 hover:to-green-700 hover:shadow-xl transition-all duration-300 flex items-center gap-2 w-full">
              <Plus className="w-5 h-5" /> New Setting
            </Button>
            <div className="flex gap-2 mt-1">
              <Link href="/dashboard/production/kanban/casting">
                <Button variant="ghost" size="icon" className="h-8 w-8 text-emerald-700 hover:bg-emerald-100 hover:text-emerald-800 rounded-full" title="Back to Casting">
                  <ArrowLeft className="w-4 h-4" />
                </Button>
              </Link>
              <Link href="/dashboard/production/kanban/polishing">
                <Button variant="ghost" size="icon" className="h-8 w-8 text-emerald-700 hover:bg-emerald-100 hover:text-emerald-800 rounded-full" title="Next to Polishing">
      ...
      <div className="relative z-10 max-w-7xl mx-auto mb-8 px-4 md:px-0">
        <Card className="p-6 shadow-xl rounded-2xl bg-white/80 border-emerald-100 flex flex-col md:flex-row md:items-end md:justify-between gap-4">
          <div className="flex flex-col md:flex-row gap-2 w-full md:w-auto">
            <Input
              placeholder="Search by customer or setting #..."
          ...
          <CardContent className="p-0">
            <div className="rounded-2xl overflow-hidden luxury-table-wrapper">
              <table className="w-full bg-white/60 luxury-table">
                ...
                <tbody>
                  {sortedSettingFiles.length === 0 ? (
                    <tr>
                      <td colSpan={9} className="text-center py-8 text-emerald-400">No setting files found.</td>
                    </tr>
                  ) : (
                    sortedSettingFiles.map(setting => (
                      <tr key={setting.settingId} className="border-b border-emerald-100 hover:bg-emerald-50/60 transition luxury-row">
                        <td className="px-2 py-3 font-semibold text-emerald-900 whitespace-nowrap">
                          <Link href={`/dashboard/production/kanban/setting/${setting.settingId}`} className="hover:text-emerald-600 transition">
                            {setting.settingId}
                          </Link>
                        </td>
                        <td className="px-2 py-3 whitespace-nowrap">
                          <Link href={`/dashboard/customers/${setting.customerId}`} className="hover:text-emerald-600 transition">
                        ...
                        <td className="px-2 py-3 whitespace-nowrap">
                          <select
                            className={`border rounded-xl px-2 py-1 text-xs font-semibold focus:border-emerald-400 focus:ring-emerald-400/20 
                              ${setting.status === 'In Progress' ? 'bg-blue-100 text-blue-800' : ''}
                              ${setting.status === 'Review' ? 'bg-yellow-100 text-yellow-800' : ''}
                              ${setting.status === 'Approved' ? 'bg-green-100 text-green-800' : ''}
                              ${setting.status === 'Revise' ? 'bg-red-300 text-red-900' : ''}`}
                            value={setting.status}
                            onChange={e => {
                              const newStatus = e.target.value;
                              setSettingOrders(prev => prev.map(s =>
                                s.settingId === setting.settingId ? { ...s, status: newStatus } : s
                              ));
                            }}
                          >
                            <option value="In Progress">In Progress</option>
                        ...
                        <td className="px-2 py-3 whitespace-nowrap">
                          <select
                            className={`border rounded-xl px-2 py-1 text-xs font-semibold focus:border-emerald-400 focus:ring-emerald-400/20 
                              ${setting.priority === 'High' ? 'bg-red-300 text-red-900' : ''}
                              ${setting.priority === 'Medium' ? 'bg-yellow-100 text-yellow-800' : ''}
                              ${setting.priority === 'Low' ? 'bg-green-100 text-green-800' : ''}`}
                            value={setting.priority}
                            onChange={e => {
                              const newPriority = e.target.value;
                              setSettingOrders(prev => prev.map(s =>
                                s.settingId === setting.settingId ? { ...s, priority: newPriority } : s
                              ));
                            }}
                          >
                        ...
                        <td className="px-2 py-3 whitespace-nowrap">
                          <div className="flex items-center gap-2">
                            <span>{setting.assignedTo}</span>
                            <Button
                              variant="ghost"
                              size="sm"
                              className="h-6 w-6 p-0 hover:bg-emerald-100 rounded-full"
                              onClick={() => handleContactAssignee(setting.assignedTo)}
                            >
                        ...
                        <td className="px-2 py-3 whitespace-nowrap text-sm">
                          <DateCell date={setting.dueDate} />
                        </td>
                        <td className="px-2 py-3 text-center whitespace-nowrap">
                          {setting.files && setting.files.length > 0 ? (
                            <div className="flex items-center justify-center gap-1">
                              <Paperclip className="h-4 w-4 text-emerald-600" />
                              <span className="text-xs text-emerald-600">{setting.files.length}</span>
                            </div>
                          ) : (
                          ...
                          <div className="flex items-center gap-2 justify-center">
                            <DropdownMenu>
                              ...
                              <DropdownMenuContent align="end" className="w-48 rounded-xl shadow-xl bg-white/95 border-emerald-200">
                                ...
                                <DropdownMenuItem className="gap-2 cursor-pointer" onClick={() => router.push(`/dashboard/production/kanban/setting/${setting.settingId}/send-to-polishing`)}>
                                  <ArrowRight className="h-4 w-4" /> Send to Polishing
                                </DropdownMenuItem>
                              </DropdownMenuContent>
                            </DropdownMenu>
                            <DropdownMenu>
                              ...
                              <DropdownMenuContent align="end" className="w-48 rounded-xl shadow-xl bg-white/95 border-emerald-200">
                                <DropdownMenuItem className="gap-2 cursor-pointer" onClick={() => setSettingOrders(prev => prev.map(s => s.settingId === setting.settingId ? { ...s, status: 'Revise' } : s))}>
                                  <AlertTriangle className="h-4 w-4" /> Revise
                                </DropdownMenuItem>
                                <DropdownMenuItem className="gap-2 cursor-pointer" onClick={() => router.push(`/dashboard/production/kanban/setting/${setting.settingId}/revisions`)}>
                                  <MessageSquare className="h-4 w-4" /> Revisions
                                </DropdownMenuItem>
                                <DropdownMenuItem className="gap-2 cursor-pointer" onClick={() => handleDownloadPDF(setting)}>
                                  <Download className="h-4 w-4" /> Download PDF
                                </DropdownMenuItem>
                                <DropdownMenuItem className="gap-2 cursor-pointer" onClick={() => handleContactAssignee(setting.assignedTo)}>
