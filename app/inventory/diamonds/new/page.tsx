...
const initialDiamond: Omit<Diamond, 'id' | 'created_at' | 'updated_at'> = {
  clarity: '',
  color: '',
  cut: '',
  carat_weight: 0,
  ...
  grading_lab: '',
  report_number: '',
  depth_percentage: undefined,
  table_percentage: undefined,
  length: undefined,
  width: undefined,
  depth: undefined,
  location: '',
...
export default function NewDiamondPage() {
  ...
  const handleChange = (field: keyof typeof diamond, value: any) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setLoading(true)
    try {
      await api.diamonds.create(diamond)
    ...
    <div className="space-y-6">
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        <div className="flex justify-end gap-4">
          ...
          <Button type="submit" disabled={loading}>{loading ? 'Saving...' : 'Create Diamond'}</Button>
        </div>
      </form>
    </div>
  )
