...
async function GET() {
  const { data, error } = await supabaseAdmin.from('accounts_payable').select('*').order('created_at', { ascending: false })
...
async function POST(req: NextRequest) {
  ...
  const { data, error } = await supabaseAdmin.from('accounts_payable').insert([body]).select('*').single()
  if (error) return NextResponse.json({ success: false, error: error.message }, { status: 500 })
  return NextResponse.json({ success: true, data })
} 
