...
async function GET(_req: NextRequest, { params }: { params: { id: string } }) {
  const { data, error } = await supabaseAdmin.from('accounts_receivable').select('*').eq('id', params.id).single()
...
async function PUT(req: NextRequest, { params }: { params: { id: string } }) {
  ...
  const { data, error } = await supabaseAdmin.from('accounts_receivable').update(body).eq('id', params.id).select('*').single()
  if (error) return NextResponse.json({ success: false, error: error.message }, { status: 500 })
  return NextResponse.json({ success: true, data })
}

async function DELETE(_req: NextRequest, { params }: { params: { id: string } }) {
  const { error } = await supabaseAdmin.from('accounts_receivable').delete().eq('id', params.id)
  if (error) return NextResponse.json({ success: false, error: error.message }, { status: 500 })
