...
async function POST(req: NextRequest) {
  ...
  let details: any = {}
  try {
    const endpoint = `https://${store_url}/admin/api/2023-04/products.json`
    ...
    shopifyProductId = data.product?.id || null
    details = { product, shopifyProductId, shopifyResponse: data }
  } catch (err: any) {
    status = 'error'
  ...
  if (status === 'error') {
    return NextResponse.json({ success: false, error: details.error || 'Shopify upload failed' }, { status: 500 })
  }
  return NextResponse.json({ success: true, shopifyProductId, shopifyResponse: details.shopifyResponse || null })
} 
