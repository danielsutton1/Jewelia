...
// POST /api/trade-ins/[id]/files - Upload files
async function POST(
  request: Request,
  ...
  try {
    ...
    const type = formData.get('type') as 'photo' | 'document'
    const files = formData.getAll('files')

    const validated = uploadFilesSchema.parse({ type, files })

    const urls = await tradeInService.uploadFiles(validated.files, validated.type)
