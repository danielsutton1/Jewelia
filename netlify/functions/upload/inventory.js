...
async function POST(request: NextRequest) {
  try {
    ...
    const validationResults = importService.validateImportData(data, 'inventory_data');
    const validRows = validationResults.filter(r => r.status === 'success').map(r => r.data);
    let processResults = [];
    if (validRows.length > 0) {
      processResults = await importService.processImportBatch(validRows, tenantId, 'inventory_data', supabase);
    }
