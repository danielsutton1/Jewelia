...
-- Content reports with enhanced structure
CREATE TABLE IF NOT EXISTS social_content_reports (
    id UUID DEFAULT gen_random_uuid() PRIMARY KEY,
    reporter_id TEXT NOT NULL,
    reported_content_type VARCHAR(20) NOT NULL CHECK (reported_content_type IN ('post', 'comment', 'user', 'community', 'event')),
    reported_content_id TEXT NOT NULL,
    report_reason VARCHAR(50) NOT NULL CHECK (report_reason IN (
        'spam', 'inappropriate', 'harassment', 'fake_news', 'copyright', 
        'commercial_solicitation', 'impersonation', 'other'
    )),
    report_details TEXT,
    report_status VARCHAR(20) DEFAULT 'pending' CHECK (report_status IN ('pending', 'investigating', 'resolved', 'dismissed')),
    moderator_id TEXT,
    moderation_notes TEXT,
    moderation_action VARCHAR(50) CHECK (moderation_action IN (
...
-- Moderation indexes
CREATE INDEX IF NOT EXISTS idx_content_reports_status ON social_content_reports(report_status);
CREATE INDEX IF NOT EXISTS idx_content_reports_reporter_id ON social_content_reports(reporter_id);
CREATE INDEX IF NOT EXISTS idx_content_reports_content_type_id ON social_content_reports(reported_content_type, reported_content_id);
...
CREATE POLICY "Moderators can view all reports" ON social_content_reports
    FOR SELECT USING (
        EXISTS (
            SELECT 1 FROM social_community_members 
...
GRANT SELECT, INSERT, UPDATE, DELETE ON social_content_reports TO authenticated;
