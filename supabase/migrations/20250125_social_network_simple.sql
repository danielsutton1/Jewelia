-- 🔧 SIMPLIFIED SOCIAL NETWORK FOUNDATION MIGRATION
-- This migration creates only the essential social networking tables
-- Avoids conflicts with existing migrations
...
CREATE POLICY IF NOT EXISTS "Users can view public profiles" ON social_profiles
    FOR SELECT USING (is_public = true OR auth.uid() = user_id);

CREATE POLICY IF NOT EXISTS "Users can update their own profile" ON social_profiles
    FOR UPDATE USING (auth.uid() = user_id);

CREATE POLICY IF NOT EXISTS "Users can insert their own profile" ON social_profiles
    FOR INSERT WITH CHECK (auth.uid() = user_id);

-- User connections policies
CREATE POLICY IF NOT EXISTS "Users can view their connections" ON user_connections
    FOR SELECT USING (
        auth.uid() = follower_id OR 
        auth.uid() = following_id
    );

CREATE POLICY IF NOT EXISTS "Users can create connections" ON user_connections
...
CREATE POLICY IF NOT EXISTS "Users can delete their connections" ON user_connections
    FOR DELETE USING (auth.uid() = follower_id OR auth.uid() = following_id);

-- Social preferences policies
CREATE POLICY IF NOT EXISTS "Users can view their own preferences" ON social_preferences
    FOR SELECT USING (auth.uid() = user_id);

CREATE POLICY IF NOT EXISTS "Users can manage their own preferences" ON social_preferences
