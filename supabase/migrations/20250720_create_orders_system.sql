...
-- Create unique constraint on order_number
...
CREATE POLICY "Enable insert access for authenticated users" ON "public"."orders" FOR INSERT WITH CHECK (auth.role() = 'authenticated');
CREATE POLICY "Enable update access for authenticated users" ON "public"."orders" FOR UPDATE USING (auth.role() = 'authenticated');
